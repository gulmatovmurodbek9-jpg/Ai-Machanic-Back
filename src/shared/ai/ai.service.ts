...
export class AiService {
  ...
  private hasOpenAiLikeKey: boolean;
  private groqApiKey: string | null = null;
  private usesGroq = false;
  ...
  private async localizeDiagnosisResult(
...
${JSON.stringify(result)}`;
    ...
    try {
      if (this.hasOpenAiLikeKey && this.openai) {
        ...
        return this.parseJson<DiagnosisResult>(completion.choices[0].message.content || '');
      }
      ...
      if (this.hasAnthropicKey && this.anthropic) {
        ...
        return this.parseJson<DiagnosisResult>(text.text);
      }
    } catch (error) {
  ...
  private mockDiagnosisResult(fileType: string, language?: SupportedLanguage): DiagnosisResult {
    const normalized = normalizeLanguage(language);

    const mocks: Record<SupportedLanguage, Record<string, DiagnosisResult>> = {
      en: {
        image: {
          problem: 'Demo mode: AI image analysis unavailable',
          diagnosis:
            'No image-analysis API key is configured, so the backend cannot inspect the uploaded photo yet. Add ANTHROPIC_API_KEY or GROQ_API_KEY or OPENAI_API_KEY in Back/.env to get a real visual diagnosis.',
          severity: 'MEDIUM',
          estimatedCostMin: 0,
          estimatedCostMax: 0,
        ...
        audio: {
          ...
          diagnosis:
            'Rhythmic knocking sound suggests pre-ignition or rod bearing wear. Could indicate low oil pressure or incorrect fuel octane.',
          severity: 'HIGH',
        ...
        video: {
          ...
          diagnosis:
            'Video shows delayed gear engagement and RPM fluctuation consistent with worn transmission clutch packs or low transmission fluid.',
          severity: 'HIGH',
      ...
      ru: {
        image: {
          ...
          diagnosis:
            'Ключ API для анализа изображений не настроен, поэтому backend пока не может проверить загруженную фотографию. Добавьте ANTHROPIC_API_KEY, GROQ_API_KEY или OPENAI_API_KEY в Back/.env для реальной визуальной диагностики.',
          severity: 'MEDIUM',
          estimatedCostMin: 0,
          estimatedCostMax: 0,
        ...
        audio: {
          ...
          diagnosis:
            'Ритмичный стук может указывать на раннее воспламенение или износ шатунных вкладышей. Также возможны низкое давление масла или неподходящее октановое число топлива.',
          severity: 'HIGH',
          estimatedCostMin: 200,
          estimatedCostMax: 1500,
        ...
        video: {
          ...
          diagnosis:
            'Видео показывает задержку включения передачи и скачки оборотов, что может говорить об износе пакетов сцепления АКПП или низком уровне трансмиссионной жидкости.',
          severity: 'HIGH',
          estimatedCostMin: 800,
          estimatedCostMax: 2500,
      ...
      tj: {
        image: {
          ...
          diagnosis:
            'Калиди API барои таҳлили сурат танзим нашудааст, бинобар ин backend ҳоло наметавонад сурати боршударо тафтиш кунад. Барои ташхиси воқеӣ ба Back/.env ANTHROPIC_API_KEY, GROQ_API_KEY ё OPENAI_API_KEY илова кунед.',
          severity: 'MEDIUM',
          estimatedCostMin: 0,
          estimatedCostMax: 0,
        ...
        audio: {
          ...
          diagnosis:
            'Садои кӯфтани ритмнок метавонад ба оташгирии пешазвақтӣ ё фарсудашавии подшипникҳои шатун ишора кунад. Инчунин метавонад аз паст будани фишори равған ё октани нодурусти сӯзишворӣ бошад.',
          severity: 'HIGH',
          estimatedCostMin: 200,
          estimatedCostMax: 1500,
        ...
        video: {
          ...
          diagnosis:
            'Дар видео дер даргир шудани фишанг ва тағйири номунтазами гардишҳо дида мешавад, ки метавонад ба фарсудашавии clutch pack-и қуттии суръат ё кам будани равғани трансмиссионӣ ишора кунад.',
          severity: 'HIGH',
          estimatedCostMin: 800,
          estimatedCostMax: 2500,
  ...
  private async analyzeImageWithGroq(imageBase64: string, carInfo: CarInfo): Promise<DiagnosisResult> {
    if (!this.groqApiKey) {
    ...
    return this.parseJson<DiagnosisResult>(content);
  }

  async analyzeImage(imageBase64: string, carInfo: CarInfo): Promise<DiagnosisResult> {
    if (this.hasAnthropicKey) {
      try {
        const message = await this.anthropic!.messages.create({
          model: 'claude-opus-4-5',
        ...
        const parsed = this.parseJson<DiagnosisResult>(text.text);
    ...
    return this.localizeDiagnosisResult(
      this.mockDiagnosisResult('image', carInfo.language),
      carInfo.language,
    );
  }

  async analyzeAudio(audioUrl: string, carInfo: CarInfo): Promise<DiagnosisResult> {
    if (!this.hasOpenAiLikeKey) {
      this.logger.warn('Using mock audio diagnosis');
      return this.localizeDiagnosisResult(
        this.mockDiagnosisResult('audio', carInfo.language),
        carInfo.language,
      );
    }

    try {
      const audioDescription = `Audio file at: ${audioUrl}. ${
        carInfo.description || 'Engine making unusual noise.'
      }`;
      ...
      const parsed = this.parseJson<DiagnosisResult>(completion.choices[0].message.content || '');
      return this.localizeDiagnosisResult(parsed, carInfo.language);
    } catch (error) {
  ...
  async analyzeVideo(videoUrl: string, carInfo: CarInfo): Promise<DiagnosisResult> {
    if (!this.hasOpenAiLikeKey) {
      this.logger.warn('Using mock video diagnosis');
      return this.localizeDiagnosisResult(
        this.mockDiagnosisResult('video', carInfo.language),
        carInfo.language,
      );
    }

    try {
      const videoDescription = `Video file at: ${videoUrl}. ${
        carInfo.description || 'Vehicle showing unusual behavior.'
      }`;
  ...
  async checkQuoteFairness(diagnosis: Diagnosis, quotedAmount: number): Promise<QuoteResult> {
    if (!this.hasOpenAiLikeKey) {
    ...
    try {
      const prompt = buildCheckQuotePrompt(
        diagnosis.carYear,
        diagnosis.carMake,
        diagnosis.carModel,
        diagnosis.problem,
        diagnosis.diagnosis,
        quotedAmount,
        Number(diagnosis.estimatedCostMin),
        Number(diagnosis.estimatedCostMax),
      );
      ...
      return this.parseJson<QuoteResult>(completion.choices[0].message.content || '');
    } catch (error) {
  ...
  async analyzeObdCodes(codes: string[]): Promise<ObdAnalysisItem[]> {
    const knownCodes: Record<
      string,
      { description: string; severity: string; recommendation: string }
    > = {
      P0300: {
    ...
    if (!this.hasOpenAiLikeKey) {
      return codes.map((code) => {
        ...
        return {
          ...
          recommendation:
            known?.recommendation || 'Consult a certified mechanic for diagnosis',
        };
      });
    }
...
Severity must be one of: LOW, MEDIUM, HIGH, CRITICAL`,
      ...
      return this.parseJson<ObdAnalysisItem[]>(completion.choices[0].message.content || '');
    } catch (error) {
  ...
  async runSecondaryAgents(
    ...
    try {
      ...
      // Agent 3: Videos (Agent 2 was the primary diagnostic)
      ...
      const parseOrEmptyArr = (res: any) => {
        if (res.status === 'fulfilled' && res.value?.choices?.[0]?.message?.content) {
          try { return this.parseJson<any[]>(res.value.choices[0].message.content); } catch { return []; }
        }
        return [];
      };

      const parseOrEmptyObj = (res: any, def: any) => {
        if (res.status === 'fulfilled' && res.value?.choices?.[0]?.message?.content) {
          try { return this.parseJson<any>(res.value.choices[0].message.content); } catch { return def; }
        }
        return def;
      };
